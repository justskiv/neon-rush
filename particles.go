...
func (ps *ParticleSystem) EmitExhaust(x, y, intensity float64, trailClr color.RGBA) {
	p := ps.spawn()
	if p == nil {
		return
	}
	...
	*p = Particle{
		...
		W: size, H: size, TTL: ttl, MaxTTL: ttl,
		Color: color.RGBA{trailClr.R, trailClr.G, trailClr.B, a}, FadeOut: true, Shrink: true,
	}
}
